...
setXbyY($sql);
        ...
        $url0 = $callback_url . "?status=" . urlencode("SUCCESS") . "&utr=" . urlencode($issuerRefNo) . "&order_id=" . urlencode($order_id);
        $data = file_get_contents($url0);
...
setXbyY($sqlv);
        ...
        $url_c = "$callback_url?status=FAILURE&utr=NILL&order_id=$order_id";
        $data1 = file_get_contents($url_c);
